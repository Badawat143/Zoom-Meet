...
export default function DirectJoinModal({
  ...
  const handleUrlChange = (value: string) => {
    ...
    // Zoom format: zoom.us/j/1234567890 or us04web.zoom.us/j/...
    else if (trimmed.includes('zoom.us') || trimmed.includes('/j/')) {
      setPlatform('zoom');
      const match = trimmed.match(/\/j\/(\d+)/);
  ...
  return (
    <div id="direct-join-modal" className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center p-4 z-50 animate-fade-in select-none">
      <div className="w-full max-w-xl bg-zinc-900 border border-zinc-700 rounded-2xl overflow-hidden shadow-2xl flex flex-col text-zinc-100">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="font-bold text-sm text-white flex items-center gap-2">
                Direct Join Any Zoom or Google Meet
                <span className="text-[10px] bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 px-2 py-0.5 rounded-full font-mono">
                  Unlimited Attendees
                </span>
              </h2>
              <p className="text-[11px] text-zinc-400">
                Paste any meeting link or Meeting ID to join immediately with customized attendee volume.
              </p>
            </div>
          </div>
          <button
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4 overflow-y-auto max-h-[80vh] text-xs">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              >
                ...
                <span>Zoom Workplace Meeting</span>
              </button>
          ...
          <div>
            ...
            <input
              type="text"
              placeholder="e.g. Global Tech Townhall, Q3 Business Review, Class 10 Lecture..."
              value={topic}
              ...
              {[50, 100, 250, 500, 1000].map((count) => (
                ...
                >
                  ...
                  <span className="text-[9px] text-zinc-500">
                    {count === 100 ? '⭐ Default' : count >= 500 ? '🔥 Mass' : 'Standard'}
                  </span>
                </button>
              ))}
            </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              <button
                ...
                className="p-2.5 rounded-lg bg-zinc-950/60 border border-zinc-800 hover:border-zinc-700 text-left transition-colors"
              >
